...
class Products extends AbstractApi
{
    ...
    public function _getResponse()
    {
        ...
        // additional params
        $order = $this->_request->getParam('order', 'desc');
        $orderBy = $this->_request->getParam('orderby', 'entity_id');
        ...
        switch ($type) {
            ...
            case self::REQUEST_SEARCH:
                if ($param) {
                    $data = $this->_searchProduct($param, $order, $orderBy, $page, $perPage);
                }
                break;
            default:
     ...
     * @return array
     */
    protected function _searchProduct($term, $order = 'desc', $orderBy = 'entity_id', $page = 1, $perPage = 10)
    {
        $products = [];
        /* @var $productCollection \Magento\Catalog\Model\ResourceModel\Product\Collection */
        $productCollection = $this->_productFactory->create()->getCollection();
